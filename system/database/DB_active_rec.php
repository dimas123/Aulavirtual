...
class CI_DB_active_record extends CI_DB_driver {
	...
	function paginar($sql, $pagina = 0, $nro_resultados = 6)
	{
		$pagina = ($pagina > 1) ? --$pagina : 0; 

		$sql = str_replace('@__',$this->dbprefix, $sql);
		$sql = preg_replace('/SELECT/','SELECT SQL_CALC_FOUND_ROWS',$sql,1); 
		$sql.= ' LIMIT ' . (($pagina /*-1*/)*$nro_resultados) . ',' . $nro_resultados . ';';

		$query_1 = $this->query($sql);
		$query_2 = $this->query('SELECT FOUND_ROWS() AS "total";');

		return array(
		'total' => $query_2->row()->total,
		'pages' => ceil($query_2->row()->total/$nro_resultados), 
